...
public class VoteBanPlugin : BasePlugin
{
   ...
   private void LoadConfig()
    ...
    {
        if (player == null || !IsAdminWithFlag(player, "@css/ban")) // Замените "admin_flag" на флаг, необходимый для использования команды
        {
   ...
   private void CommandVote(CCSPlayerController? player, CommandInfo commandInfo, Action<string> executeAction)
    ...
    {
        var voteMenu = _api.GetMenu("Голосуйте за игрока");
